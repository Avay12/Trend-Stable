...
@Component({
  selector: 'app-cheap-order-service',
   imports: [CommonModule, FormsModule, ReactiveFormsModule, RouterLink, DecimalPipe],
  templateUrl: './cheap-order-service.html',
  styleUrl: './cheap-order-service.css',
})
export class CheapOrderService {
 providerService = inject(ProviderServices);
  ...
  // ... (lines 48-223 are unchanged, but I need to be careful with context)
  ...
  loadServices() {
    ...
    this.providerService.getCheapServices(platform).subscribe({
      next: (services) => {
