...
export class NewOrder {
  ...
  categoryCounts = computed(() => {
   ...
   bestSelling = signal([
    ...
    { name: 'Instagram Video Views | Speed: 5-50K/Day | Max 10M', orders: 6689, rate: 93, type: 'Elite' },
    { name: 'YouTube Real Likes | Speed 10-20K/Day | Max 150K', orders: 6652, rate: 93, type: 'Basic' },
    ...
    { name: 'TikTok Video Views | 1-5M/Day | Max 10M', orders: 4795, rate: 97, type: 'Elite' },
  ...
  selectTopCategory(category: Category) {
    ...
    this.providerService.getServices(platform).subscribe(services => {
      this.providerService.services.set(services);
      this.isLoadingServices.set(false);
    ...
    this.orderService
      ...
      .subscribe({
        next: (order) => {
          ...
          // Construct the full order object from local state + API response (ID)
          // because API might not return the full details or they might be empty
          const fullOrder: any = {
            id: order.id, // Order ID from API
